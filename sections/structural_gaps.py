...
def render_structural_gaps(filtered_df, state_stats_raw, sub_text, get_k_color):
    ...
    state_agg = filtered_df.groupby("State").agg(
        ...
        Total_ICU_Beds=("icu_beds_estimated", "sum"),
    ...
    # ============================================================
    # STEP 3: Deficit % (Strictly 0-100. 0 means goal met)
    # ============================================================
    ...
    state_agg["ICU_Deficit"] = (((state_agg["Req_ICU"] - state_agg["Total_ICU_Beds"]) / state_agg["Req_ICU"]) * 100).clip(0, 100)
    state_agg["ER_Deficit"] = (((state_agg["Req_ER"] - state_agg["Total_Emergency"]) / state_agg["Req_ER"]) * 100).clip(0, 100)

    # Calculate Adequacy (for the 'Strength' chart)
    state_agg["Bed_Adequacy"] = (state_agg["Total_Beds"] / state_agg["Req_Beds"] * 100)
    state_agg["Doc_Adequacy"] = (state_agg["Total_Doctors"] / state_agg["Req_Docs"] * 100)
    state_agg["ICU_Adequacy"] = (state_agg["Total_ICU_Beds"] / state_agg["Req_ICU"] * 100)
    state_agg["ER_Adequacy"] = (state_agg["Total_Emergency"] / state_agg["Req_ER"] * 100)

    # ============================================================
    # STEP 4: SDS = WEIGHTED average of deficits
    # ============================================================
    state_agg["SDS"] = (
    ...
    # ============================================================
    # NATIONAL KPIs (Average of State Deficits)
    # ============================================================
    nat_bed_def = state_agg["Bed_Deficit"].mean()
    nat_doc_def = state_agg["Doc_Deficit"].mean()
    nat_icu_def = state_agg["ICU_Deficit"].mean()
    nat_er_def = state_agg["ER_Deficit"].mean()
    ...
    # ============================================================
    # ROW 3: GRAPH 2 — Top 10 Structurally Strong States (Showing Adequacy)
    # ============================================================
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
    st.markdown('<div class="card-header">Top 10 States \u2013 Systemic Resource Adequacy (Goal Benchmarking)</div>', unsafe_allow_html=True)
    ...
    fig_strong.add_trace(go.Bar(name="Bed Capacity %", x=top10_strong["State"], y=top10_strong["Bed_Adequacy"], marker_color="#10b981"))
    fig_strong.add_trace(go.Bar(name="Doctor Capacity %", x=top10_strong["State"], y=top10_strong["Doc_Adequacy"], marker_color="#06b6d4"))
    fig_strong.add_trace(go.Bar(name="ICU Capacity %", x=top10_strong["State"], y=top10_strong["ICU_Adequacy"], marker_color="#6366f1"))
    fig_strong.add_trace(go.Bar(name="Emergency Capacity %", x=top10_strong["State"], y=top10_strong["ER_Adequacy"], marker_color="#38bdf8"))
    fig_strong.update_layout(
        barmode="group", height=400,
        yaxis=dict(title="Adequacy % (Target=100%)", ticksuffix="%"),
        margin=dict(l=0, r=0, t=30, b=0),
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color=sub_text, size=18),
        legend=dict(orientation="h", y=1.12, x=0.5, xanchor="center", font=dict(size=16, family="Plus Jakarta Sans", weight="bold"))
    )
    fig_strong.add_hline(y=100, line_dash="dash", line_color="rgba(16, 185, 129, 0.5)", annotation_text="WHO BMRK")

...
def load_data(uploaded_file=None):
    ...
    if "Facilities" in df.columns:
        ...
        # Estimate ICU beds: 5% of total beds if ICU facility exists, else 0
        df["icu_beds_estimated"] = df.apply(lambda x: x["Total_Num_Beds"] * 0.05 if x["has_icu"] else 0, axis=1)
    else:
        df["has_icu"] = False
        df["icu_beds_estimated"] = 0
